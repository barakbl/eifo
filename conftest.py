...
Settings read ``config/tvil.toml`` by default, and a checkout that has followed
...
set ``TVIL_CONFIG_FILE`` themselves, which still wins over this.
"""
...
def isolated_configuration(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Iterator[None]:
    monkeypatch.setenv("TVIL_CONFIG_FILE", str(tmp_path / "no-such-config.toml"))
