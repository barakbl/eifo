...
from tvil_fetcher.robots import RobotsDisallowedError, RobotsPolicy
...
def policy(text: str = REAL_RULES, *, user_agent: str = "tvil-fetcher/0.1") -> RobotsPolicy:
...
class TestMissingRobots:
    def test_an_unreachable_robots_file_allows_everything(self) -> None:
        ...
        assert RobotsPolicy("tvil", fetch=explode).allows("https://example.com/anything")
...
class TestCaching:
    def test_robots_is_read_once_per_host(self) -> None:
        ...
        checker = RobotsPolicy("tvil", fetch=counting_fetch)
