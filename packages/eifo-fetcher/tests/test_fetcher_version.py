...
PYPROJECT = Path(eifo_fetcher.__file__).resolve().parents[2] / "pyproject.toml"


def test_the_reported_version_is_the_installed_one() -> None:
    """The check that holds however the package was installed."""
    assert eifo_fetcher.__version__ == metadata.version("eifo-fetcher")
...
def test_the_reported_version_is_the_declared_one() -> None:
    ...
    assert eifo_fetcher.__version__ == declared
...
def test_it_is_not_the_placeholder_it_used_to_be_stuck_on() -> None:
    ...
    assert eifo_fetcher.__version__ != "0.0.0+unknown", "this package is not installed"
