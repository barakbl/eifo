...
from tvil_api.oauth import GoogleProvider, XProvider
from tvil_api.security import OAUTH_COOKIE, SESSION_COOKIE
...
class TestLoginRedirect:
    ...
    def test_the_callback_url_matches_the_deployment(self, client: TestClient) -> None:
        ...
        assert query["redirect_uri"] == ["https://tvil.test/api/v1/auth/callback/google"]
...
class TestGoogleCallback:
    ...
    def test_creates_the_account_and_opens_a_session(
        ...
        assert response.status_code == 302
        assert response.headers["location"].startswith("https://tvil.test/")
    ...
    def test_a_cancelled_sign_in_goes_back_to_the_app(self, client: TestClient) -> None:
        ...
        assert response.status_code == 302
        assert response.headers["location"] == "https://tvil.test/#/?login=cancelled"
    ...
    def test_a_provider_failure_is_reported_as_such(self, client: TestClient) -> None:
        ...
        assert response.headers["location"] == "https://tvil.test/#/?login=failed"
...
class TestUnconfiguredDeployment:
    ...
    def test_login_reports_what_is_missing(self, client: TestClient) -> None:
        ...
        assert "TVIL_SECRET_KEY" in response.json()["detail"]
