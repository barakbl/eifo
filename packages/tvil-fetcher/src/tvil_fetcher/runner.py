...
from tvil_fetcher.registry import discover_plugins, enabled_sources, plugins_for
from tvil_fetcher.sources.base import FetchContext, SourcePlugin
from tvil_fetcher.tmdb import TmdbClient

logger = logging.getLogger("tvil.fetch.runner")
...
def _tmdb_client(http: HttpClient, settings: Settings) -> TmdbClient | None:
    ...
    if settings.tmdb_api_key is None:
        logger.warning("TVIL_TMDB_API_KEY is not set; TMDB matching is disabled")
        return None
