...
from tvil_core.settings import (
...
def _write_config(tmp_path: Path, body: str) -> Path:
    path = tmp_path / "tvil.toml"
    path.write_text(body, encoding="utf-8")
    return path


def test_defaults_apply_without_any_config(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TVIL_CONFIG_FILE", "does-not-exist.toml")

    settings = Settings(_env_file=None)

    assert settings.db_url == "sqlite:///data/tvil.db"
...
def test_toml_file_overrides_defaults(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("TVIL_CONFIG_FILE", str(config))
...
def test_environment_beats_the_toml_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("TVIL_CONFIG_FILE", str(config))
    monkeypatch.setenv("TVIL_DB_URL", "sqlite:///from-env.db")
...
def test_nested_settings_come_from_the_environment(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TVIL_CONFIG_FILE", "does-not-exist.toml")
    monkeypatch.setenv("TVIL_SCORES__LOW_VOTE_THRESHOLD", "10")
...
class TestRequire:
    ...
    def test_lists_every_missing_setting_at_once(self) -> None:
        ...
        assert "TVIL_SECRET_KEY" in message
        assert "TVIL_TMDB_API_KEY" in message
...
def test_get_settings_is_cached(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("TVIL_CONFIG_FILE", "does-not-exist.toml")
    monkeypatch.setenv("TVIL_PUBLIC_ORIGIN", "https://first.example")

    first = get_settings()
    monkeypatch.setenv("TVIL_PUBLIC_ORIGIN", "https://second.example")
    second = get_settings()
