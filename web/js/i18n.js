/* Hebrew and English strings, and the direction that goes with them.
 *
 * Hebrew is the default: this is an Israeli catalog, and most of what it holds
 * has no English name at all.
 */

export const LANGUAGES = ["he", "en"];
export const DEFAULT_LANGUAGE = "he";

const STRINGS = {
  he: {
    "app.name": "איפה",
    "app.tagline": "מה זמין לצפייה בישראל",
    "search.placeholder": "חיפוש סרטים וסדרות",
    "search.shortcut": "/",
    "search.label": "חיפוש",

    "filters.services": "שירותים",
    "filters.servicesAll": "כל השירותים",
    "filters.servicesSome": "{count} שירותים",
    "filters.selectAll": "בחירת הכול",
    "filters.clear": "ניקוי",
    "filters.all": "הכול",
    "filters.type.any": "הכול",
    "filters.type.movie": "סרטים",
    "filters.type.series": "סדרות",
    "filters.sort.": "מומלץ",
    "filters.sort.score": "לפי דירוג",
    "filters.sort.score_israeli": "לפי דירוג ישראלי",
    "filters.sort.year": "לפי שנה",
    "filters.sort.name": "לפי שם",
    "filters.sort.recently_added": "נוספו לאחרונה",
    "filters.available.current": "זמין עכשיו",
    "filters.available.any": "הכול, כולל מה שירד",
    "filters.available.gone": "ירד מהאוויר",

    "results.count": "{count} כותרים",
    "results.countLabel": "כותרים",
    "results.searching": "מחפש…",

    "empty.title": "אין תוצאות",
    "empty.body": "נסו שם אחר, או הסירו חלק מהסינונים.",
    "empty.clear": "נקו סינונים",

    "error.title": "משהו השתבש",
    "error.body": "לא הצלחנו לטעון את הקטלוג.",
    "error.retry": "נסו שוב",
    "error.offline": "אין חיבור לרשת.",

    "title.notFound": "הכותר לא נמצא",
    "title.back": "חזרה לקטלוג",
    "title.overview": "תקציר",
    "title.ratings": "דירוגים",
    "title.aggregate": "ציון משוקלל",
    "title.aggregateIsraeli": "ציון ישראלי",
    "title.components": "איך חושב הציון",
    "title.provider": "מקור",
    "title.normalized": "מנורמל",
    "title.weight": "משקל",
    "title.notCountedTooFewVotes": "לא נספר - מעט מדי מצביעים",
    "score.thinVotes": "מבוסס על {votes} הצבעות בלבד",
    "title.noScore": "אין ציון",
    "title.noScoreThinVotes": "מעט מדי הצבעות מכדי לחשב ציון משוקלל",
    "title.noScoreOneRater": "דירוג יחיד אינו ציון משוקלל",
    "title.pulledToOrdinary": "הדירוגים נתנו בממוצע {mean}. עם {votes} הצבעות בלבד מאחורי זה, הציון המוצג נמשך אל הממוצע של הקטלוג.",
    "title.countedHalfThinVotes": "נספר חצי - מעט מצביעים",
    "title.votes": "הצבעות",
    "title.whereToWatch": "איפה לצפות",
    "title.watch": "לצפייה",
    "title.seasons": "{count} עונות",
    "title.runtime": "{count} דק׳",
    "title.credits": "מי עשה את זה",
    "runtime.short": "קצר",
    "runtime.standard": "רגיל",
    "runtime.long": "ארוך",
    "runtime.epic": "ארוך מאוד",
    "title.language": "שפה",
    "title.country": "ארץ הפקה",
    "title.length": "אורך",
    "credit.director": "בימוי",
    "credit.director_plural": "בימוי",
    "credit.cinematographer": "צילום",
    "credit.cinematographer_plural": "צילום",
    "credit.cast": "שחקנים",
    "credit.showAll": "כל השחקנים ({count})",
    "credit.showFewer": "פחות",
    "person.notFound": "לא מצאנו את מי שחיפשת",
    "person.credits": "{count} כותרים בקטלוג",
    "person.asDirector": "בימוי",
    "person.asCinematographer": "צילום",
    "person.asCast": "משחק",
    "person.profileOn": "העמוד ב{site}",
    "title.noRatings": "אין עדיין דירוגים לכותר הזה.",
    "title.noOffers": "לא זמין כרגע באף שירות שאנחנו עוקבים אחריו.",

    "offer.verified": "נבדק ב־{date}",
    "offer.gone": "ירד מהאוויר",
    "offer.goneSince": "ירד ב־{date}",
    "offer.untracked": "המקור אינו נתמך עוד",
    "offer.free": "חינם",
    "offer.stream": "במנוי",
    "offer.rent": "השכרה",
    "offer.buy": "רכישה",

    "theme.toggle": "החלפת ערכת נושא",
    "lang.toggle": "EN",
    "footer.data": "נתונים",

    "auth.signIn": "התחברות",
    "auth.signInWith": "התחברות עם {provider}",
    "auth.tryAnotherAccount": "ניסיון עם חשבון {provider} אחר",
    "auth.provider.google": "Google",
    "auth.provider.x": "X",
    "auth.signOut": "התנתקות",
    "auth.signOutConfirm": "להתנתק?",
    "auth.signOutConfirmBody": "כדי לחזור לרשימה שלך צריך להתחבר שוב.",
    "auth.signOutCancel": "ביטול",
    "auth.account": "החשבון שלי",
    "auth.cancelled": "ההתחברות בוטלה.",
    "auth.failed": "ההתחברות נכשלה. נסו שוב.",
    "auth.notInvited": "האתר הזה בהזמנה בלבד. בקשו מבעל האתר להוסיף את הכתובת שלכם.",
    "auth.unavailable": "התחברות אינה זמינה בהתקנה הזו.",
    "auth.menu": "תפריט חשבון",

    "suggest.titles": "כותרים",
    "suggest.people": "אנשים",
    "suggest.credits": "{count} כותרים",
    "suggest.empty": "אין הצעות",
    "filters.order.desc": "מהגבוה לנמוך — לחצו כדי להפוך",
    "filters.order.asc": "מהנמוך לגבוה — לחצו כדי להפוך",
    "filters.more": "עוד מסננים",
    "filters.moreSome": "עוד מסננים ({count})",
    "filters.length": "אורך",
    "filters.runtime.90": "עד שעה וחצי",
    "filters.runtime.120": "עד שעתיים",
    "filters.runtime.150": "עד שעתיים וחצי",
    "filters.lengthNote": "סרטים בלבד — אורך של סדרה נמדד לפי פרק. סרטים שאורכם לא ידוע לא ייכללו.",
    "filters.years": "שנים",
    "filters.year.yearMin": "משנה",
    "filters.year.yearMax": "עד שנה",
    "filters.year.fromShort": "מ־",
    "filters.year.toShort": "עד",
    "filters.yearsNote": "כותרים שהשנה שלהם לא ידועה לא ייכללו.",
    "filters.decade.2020s": "שנות ה־20",
    "filters.decade.2010s": "שנות ה־10",
    "filters.decade.2000s": "שנות ה־2000",
    "filters.decade.1990s": "שנות ה־90",
    "filters.decade.1980s": "שנות ה־80",
    "filters.decade.older": "ותיקים יותר",
    "filters.genres": "ז'אנרים",
    "filters.score": "דירוג",
    "filters.scoreAny": "כל דירוג",
    "filters.scoreAtLeast": "{score} ומעלה",
    "filters.scoreNote": "כותרים שאין להם דירוג לא ייכללו.",
    "filters.whatsNew": "מה חדש",
    "filters.stats": "סטטיסטיקה",
    "stats.title": "סטטיסטיקה",
    "stats.tab.growth": "גדילה לאורך זמן",
    "stats.tab.services": "לפי שירות",
    "stats.view.table": "טבלה",
    "stats.view.graph": "גרף",
    "stats.metric.offers": "נוספו לשירות",
    "stats.metric.titles": "חדשים בקטלוג",
    "stats.range.30": "30 יום",
    "stats.range.90": "90 יום",
    "stats.range.365": "שנה",
    "stats.range.3650": "הכול",
    "stats.loads": "כולל טעינות קטלוג",
    "stats.growth.note": "\"נוספו לשירות\" סופר כל כותר שהופיע בשירות, גם אם כבר היה בשירות אחר. \"חדשים בקטלוג\" סופר רק כותרים שלא הכרנו בכלל, ונזקף לשירות שמצא אותם ראשון. סנכרונים שהוסיפו רבע או יותר ממה שראו מוסתרים כברירת מחדל: אלה טעינות של קטלוג שכבר היה קיים (סריקה מלאה ראשונה, או סריקה שהורחבה), לא תוספות אמיתיות.",
    "stats.growth.empty": "אין סנכרונים בטווח הזה",
    "stats.growth.emptyBody": "נסו טווח ארוך יותר, או לכלול טעינות קטלוג.",
    "stats.growth.byService": "לפי שירות",
    "stats.growth.recent": "סנכרונים אחרונים",
    "stats.growth.recentNote": "{shown} האחרונים מתוך {total}.",
    "stats.graph.note": "לכל שירות סולם משלו, כדי ששירות קטן לא ייבלע ליד גדול. המספר בכותרת הוא הסכום בטווח.",
    "stats.graph.peak": "שיא {value}",
    "stats.graph.bucket.day": "ביום",
    "stats.graph.bucket.week": "בשבוע",
    "stats.graph.bucket.month": "בחודש",
    "stats.graph.quiet": "{count} שירותים בלי תוספות בטווח לא מוצגים.",
    "stats.col.service": "שירות",
    "stats.col.syncs": "סנכרונים",
    "stats.col.offersAdded": "נוספו לשירות",
    "stats.col.titlesCreated": "חדשים בקטלוג",
    "stats.col.retired": "ירדו",
    "stats.col.lastSync": "סנכרון אחרון",
    "stats.col.when": "מתי",
    "stats.col.status": "מצב",
    "stats.col.titles": "כותרים",
    "stats.col.movies": "סרטים",
    "stats.col.series": "סדרות",
    "stats.col.offers": "הצעות",
    "stats.col.stream": "במנוי",
    "stats.col.free": "חינם",
    "stats.col.rent": "השכרה",
    "stats.col.buy": "קנייה",
    "stats.col.deal": "סוג",
    "stats.col.priced": "עם מחיר",
    "stats.col.unpriced": "בלי מחיר",
    "stats.col.coverage": "כיסוי",
    "stats.col.titlesPriced": "כותרים עם מחיר",
    "stats.total": "סה\"כ",
    "stats.loadBadge": "טעינת קטלוג",
    "stats.services.note": "כותר שזמין גם להשכרה וגם לקנייה הוא כותר אחד ושתי הצעות. כותר שנמצא בכמה שירותים נספר פעם אחת בסיכום.",
    "stats.prices": "מחירים",
    "stats.prices.note": "כמה מההצעות להשכרה ולקנייה כוללות מחיר. רק שירותים שמשכירים או מוכרים כותרים מופיעים כאן.",
    "stats.prices.empty": "אף שירות לא משכיר או מוכר כותרים כרגע.",
    "whatsnew.title": "מה חדש",
    "whatsnew.note": "מה נוסף לאחרונה לשירותים, החדש ביותר קודם. סרט שכבר היה בשירות אחר נחשב חדש בשירות שהוסיף אותו.",
    "whatsnew.on": "חדש ב־{source}",
    "whatsnew.allServices": "כל השירותים",
    "whatsnew.empty": "אין כאן חדש",
    "whatsnew.emptyBody": "עוד לא ראינו תוספות בשירות הזה. נסו שירות אחר.",
    "filters.myServices": "השירותים שלי",
    "filters.myServicesEmpty": "בחרו את השירותים שלכם",
    "filters.editMine": "עריכת השירותים שלי",
    "filters.editingMine": "סמנו את השירותים שיש לכם. כל שינוי נשמר מיד.",
    "filters.done": "סיום",

    "item.watched": "נצפה",
    "item.wantToWatch": "לצפייה בהמשך",
    "item.rating": "הדירוג שלי",
    "item.ratingClear": "ניקוי דירוג",
    "item.ratingNone": "עוד לא דירגתם",
    "item.rate": "דירוג {value} מתוך 10",
    "item.note": "פתק פרטי",
    "item.notePlaceholder": "רק לעיניכם.",
    "item.noteSave": "שמירה",
    "item.noteSaved": "נשמר",
    "item.saveFailed": "השינוי לא נשמר.",
    "item.signInToTrack": "התחברו כדי לשמור רשימות ודירוגים.",
    "item.remove": "הסרה מהרשימות",

    "mylist.title": "הרשימה שלי",
    "mylist.tab.want": "לצפייה בהמשך",
    "mylist.tab.watched": "נצפו",
    "mylist.tab.rated": "דירגתי",
    "mylist.empty": "עדיין אין כאן כלום",
    "mylist.emptyBody": "סמנו כותרים כ״לצפייה בהמשך״ והם יופיעו כאן.",
    "mylist.browse": "לקטלוג",
    "mylist.where": "איפה אפשר לראות את אלה",
    "mylist.whereOf": "{count} מתוך {total}",

    "settings.title": "הגדרות",
    "settings.services": "השירותים שלי",
    "settings.servicesHelp": "משמש לסינון ״השירותים שלי״ בקטלוג.",
    "settings.profile": "פרופיל",
    "settings.displayName": "שם תצוגה",
    "settings.handle": "כינוי",
    "settings.handleHelp": "אותיות קטנות באנגלית, ספרות וקו תחתון.",
    "settings.public": "פרופיל ציבורי",
    "settings.publicHelp":
      "כשהפרופיל ציבורי, כל מי שיש לו את הקישור רואה: שם תצוגה, תמונה, הרשימות והדירוגים שלכם. " +
      "לעולם לא: כתובת המייל, זהות ההתחברות, הפתקים או בחירת השירותים.",
    "settings.save": "שמירה",
    "settings.saved": "נשמר",
    "settings.danger": "מחיקת חשבון",
    "settings.dangerBody":
      "מחיקה מיידית של החשבון, הרשימות והדירוגים. אין תקופת חסד ואי אפשר לבטל.",
    "settings.delete": "מחיקת החשבון",
    "settings.deleteConfirm": "להקליד DELETE כדי לאשר",
    "settings.deleteWord": "DELETE",

    "manage.title": "ניהול",
    "manage.tab.overview": "סקירה",
    "manage.tab.runs": "ריצות",
    "manage.tab.review": "תור בדיקה",
    "manage.tab.members": "משתמשים",
    "members.title": "מי יכול להיכנס",
    "members.explain": "רק כתובות ברשימה הזאת יכולות להתחבר. כתובת שמופיעה בקובץ ההגדרות תמיד מנהלת, ואי אפשר לשנות אותה מכאן.",
    "members.none": "עוד לא הוזמן אף אחד.",
    "members.email": "כתובת אימייל",
    "members.emailPlaceholder": "name@example.com",
    "members.role": "תפקיד",
    "members.roleMember": "צופה",
    "members.roleAdmin": "מנהל",
    "members.invite": "הזמנה",
    "members.inviteFailed": "לא הצלחנו להוסיף את הכתובת.",
    "members.promote": "הפיכה למנהל",
    "members.demote": "הסרת ניהול",
    "members.remove": "הסרה",
    "members.fromConfig": "מקובץ ההגדרות",
    "members.you": "זה אתם",
    "members.wallTitle": "הקטלוג הזה למשתמשים רשומים",
    "members.refusedTitle": "הכתובת שלכם אינה ברשימה",
    "members.refusedBody": "ההתחברות עצמה הצליחה, אבל הקטלוג הזה בהזמנה בלבד והכתובת שלכם לא נוספה אליו. בקשו מבעל האתר להוסיף אותה - התחברות נוספת עם אותו חשבון לא תשנה את זה.",
    "members.wallBody": "התחברו כדי לראות מה זמין לצפייה. אם אין לכם גישה, בקשו מבעל האתר להוסיף את הכתובת שלכם.",
    "tokens.title": "טוקנים ל-API",
    "tokens.explain": "טוקן מאפשר לסקריפט לגשת ל-API בשמכם. הוא מוצג פעם אחת בלבד.",
    "tokens.none": "אין לכם טוקנים.",
    "tokens.name": "למה הוא משמש",
    "tokens.namePlaceholder": "הסקריפט במחשב הנייד",
    "tokens.create": "יצירת טוקן",
    "tokens.created": "העתיקו אותו עכשיו - לא נוכל להציג אותו שוב.",
    "tokens.revoke": "ביטול",
    "tokens.neverUsed": "לא היה בשימוש",
    "tokens.lastUsed": "בשימוש אחרון {when}",
    "manage.denied": "האזור הזה סגור",
    "manage.deniedBody": "רק מנהלי המופע הזה יכולים לפתוח אותו.",

    "manage.stat.titles": "כותרים",
    "manage.stat.scored": "עם ציון",
    "manage.stat.withPoster": "עם פוסטר",
    "manage.stat.queueCleared": "התור טופל",
    "manage.stat.ofTitles": "{count} מתוך {total}",
    "manage.stat.missing": "{count} חסרים",
    "manage.stat.waiting": "{count} ממתינים",
    "manage.col.source": "מקור",
    "manage.col.titles": "כותרים",
    "manage.col.poster": "פוסטר",
    "manage.col.score": "ציון",
    "manage.col.queue": "טופל",
    "manage.col.enriched": "הועשר",
    "manage.source.waitingCount": "{count} ממתינים לבדיקה",
    "manage.source.queued": "סנכרון בתור",
    "manage.stat.people": "אנשים",
    "manage.stat.available": "זמינים עכשיו",
    "manage.stat.offers": "דרכים לצפות",
    "manage.stat.pending": "ממתינים לבדיקה",
    "manage.stat.stale": "מקורות לא טריים",
    "manage.stat.never": "מעולם",
    "manage.staleOf": "{stale} מתוך {total}",

    "manage.scoring": "מרכיבי הציון",
    "manage.scoring.blurb":
      "כמה כל מדרג באמת שקל בציונים שבקטלוג: המשקל שלו, כפול מספר הכותרים "
      + "המדורגים שהספיק לדרג. דירוג עם מעט מצביעים נספר חצי, בדיוק כמו בחישוב עצמו.",
    "manage.scoring.empty": "עוד אין ציון לאף כותר, אז אין ממה לגזור חלקים.",
    "manage.scoring.israeli": "ציון ישראלי",
    "manage.col.provider": "מדרג",
    "manage.col.weight": "משקל",
    "manage.col.share": "חלק בציון",
    "manage.col.covered": "כיסוי הקטלוג",
    "manage.scoring.unused": "לא דירג כלום",

    "manage.sources": "מקורות",
    "manage.source.coverage": "כיסוי",
    "manage.source.lastSync": "סנכרון אחרון",
    "manage.source.pending": "בתור",
    "manage.source.enabled": "פעיל",
    "manage.source.on": "דלוק",
    "manage.source.off": "כבוי",
    "manage.source.fromConfig": "לא נקבע ידנית",
    "manage.source.override": "נקבע ידנית",
    "manage.source.clearOverride": "החזרה לקובץ",
    "manage.source.stale": "לא טרי",
    "manage.source.retired": "לא נאסף יותר",
    "manage.source.nextRun": "ייכנס לתוקף בריצה הבאה",

    "manage.runs.phase": "שלב",
    "manage.runs.status": "מצב",
    "manage.runs.started": "התחיל",
    "manage.runs.duration": "משך",
    "manage.runs.source": "מקור",
    "manage.runs.log": "יומן",
    "manage.runs.showLog": "הצגת היומן",
    "manage.runs.hideLog": "סגירה",
    "manage.runs.noLog": "לריצה הזאת אין יומן",
    "manage.runs.empty": "עוד לא רצה אף ריצה",
    "manage.runs.all": "הכול",
    "manage.runs.more": "עוד",

    "review.title": "תור בדיקה",
    "review.empty": "התור ריק",
    "review.emptyBody": "כל מה שהמקורות הציעו נמצא בקטלוג.",
    "review.order.age": "הישנים קודם",
    "review.order.similarity": "לפי קרבה",
    "review.allSources": "כל המקורות",
    "review.source": "המקור מציע",
    "review.candidate": "אולי זה",
    "review.noCandidate": "לא נמצא כותר דומה",
    "review.kind.movie": "סרט",
    "review.kind.series": "סדרה",
    "review.similarity": "{value}% התאמה",
    "review.same": "אותו כותר",
    "review.different": "כותר חדש",
    "review.junk": "לא כותר",
    "review.open": "בדף המקור",
    "review.remaining": "{count} ממתינים",
    "review.ruled": "טופל",
    "review.failed": "לא הצלחנו לשמור את ההחלטה",
    "review.selected": "{count} נבחרו",
    "review.bulkDismiss": "סימון הכול כלא-כותר",
    "review.bulkCreate": "יצירת כותר לכל אחד",
    "review.clearSelection": "ביטול הבחירה",
    "review.select": "בחירה",
    "review.keys": "1 אותו כותר · 2 כותר חדש · 3 לא כותר · j/k מעבר",
  },
  en: {
    "app.name": "Eifo",
    "app.tagline": "What's streaming in Israel",
    "search.placeholder": "Search films and series",
    "search.shortcut": "/",
    "search.label": "Search",

    "filters.services": "Services",
    "filters.servicesAll": "All services",
    "filters.servicesSome": "{count} services",
    "filters.selectAll": "Select all",
    "filters.clear": "Clear",
    "filters.all": "All",
    "filters.type.any": "All",
    "filters.type.movie": "Films",
    "filters.type.series": "Series",
    "filters.sort.": "Recommended",
    "filters.sort.score": "By score",
    "filters.sort.score_israeli": "By Israeli score",
    "filters.sort.year": "By year",
    "filters.sort.name": "By name",
    "filters.sort.recently_added": "Recently added",
    "filters.available.current": "Available now",
    "filters.available.any": "Everything, including gone",
    "filters.available.gone": "No longer available",

    "results.count": "{count} titles",
    "results.countLabel": "titles",
    "results.searching": "Searching…",

    "empty.title": "Nothing matched",
    "empty.body": "Try a different name, or clear some filters.",
    "empty.clear": "Clear filters",

    "error.title": "Something went wrong",
    "error.body": "The catalog could not be loaded.",
    "error.retry": "Try again",
    "error.offline": "You are offline.",

    "title.notFound": "Title not found",
    "title.back": "Back to the catalog",
    "title.overview": "Overview",
    "title.ratings": "Ratings",
    "title.aggregate": "Weighted score",
    "title.aggregateIsraeli": "Israeli score",
    "title.components": "How this score was computed",
    "title.provider": "Provider",
    "title.normalized": "Normalised",
    "title.weight": "Weight",
    "title.notCountedTooFewVotes": "not counted - too few votes",
    "score.thinVotes": "based on only {votes} votes",
    "title.noScore": "No score",
    "title.noScoreThinVotes": "too few votes to average",
    "title.noScoreOneRater": "one rating is not an average",
    "title.pulledToOrdinary": "The raters averaged {mean}. With only {votes} votes behind that, the score shown is pulled towards the catalog average.",
    "title.countedHalfThinVotes": "counted half - thin votes",
    "title.votes": "Votes",
    "title.whereToWatch": "Where to watch",
    "title.watch": "Watch",
    "title.seasons": "{count} seasons",
    "title.runtime": "{count} min",
    "title.credits": "Who made it",
    "runtime.short": "Short",
    "runtime.standard": "Standard",
    "runtime.long": "Long",
    "runtime.epic": "Epic",
    "title.language": "Language",
    "title.country": "Country",
    "title.length": "Length",
    "credit.director": "Director",
    "credit.director_plural": "Directors",
    "credit.cinematographer": "Cinematography",
    "credit.cinematographer_plural": "Cinematography",
    "credit.cast": "Cast",
    "credit.showAll": "All cast ({count})",
    "credit.showFewer": "Show fewer",
    "person.notFound": "We could not find that person",
    "person.credits": "{count} titles in the catalog",
    "person.asDirector": "Directed",
    "person.asCinematographer": "Shot",
    "person.asCast": "Acted in",
    "person.profileOn": "Their page on {site}",
    "title.noRatings": "No ratings for this title yet.",
    "title.noOffers": "Not currently on any service we track.",

    "offer.verified": "Verified {date}",
    "offer.gone": "No longer available",
    "offer.goneSince": "Gone since {date}",
    "offer.untracked": "Source no longer tracked",
    "offer.free": "Free",
    "offer.stream": "Subscription",
    "offer.rent": "Rent",
    "offer.buy": "Buy",

    "theme.toggle": "Toggle theme",
    "lang.toggle": "עב",
    "footer.data": "Data",

    "auth.signIn": "Sign in",
    "auth.signInWith": "Sign in with {provider}",
    "auth.tryAnotherAccount": "Try a different {provider} account",
    "auth.provider.google": "Google",
    "auth.provider.x": "X",
    "auth.signOut": "Sign out",
    "auth.signOutConfirm": "Sign out?",
    "auth.signOutConfirmBody": "You will need to sign in again to get back to your list.",
    "auth.signOutCancel": "Cancel",
    "auth.account": "My account",
    "auth.cancelled": "Sign-in was cancelled.",
    "auth.failed": "Sign-in failed. Please try again.",
    "auth.notInvited": "This site is invite-only. Ask the owner to add your address.",
    "auth.unavailable": "Sign-in is not available on this installation.",
    "auth.menu": "Account menu",

    "suggest.titles": "Titles",
    "suggest.people": "People",
    "suggest.credits": "{count} titles",
    "suggest.empty": "No suggestions",

    "filters.order.desc": "Highest first \u2014 click to reverse",
    "filters.order.asc": "Lowest first \u2014 click to reverse",
    "filters.more": "More filters",
    "filters.moreSome": "More filters ({count})",
    "filters.length": "Length",
    "filters.runtime.90": "Up to 1\u00bd hrs",
    "filters.runtime.120": "Up to 2 hrs",
    "filters.runtime.150": "Up to 2\u00bd hrs",
    "filters.lengthNote":
      "Films only \u2014 a series' length is per episode. Films with no known runtime are left out.",
    "filters.years": "Years",
    "filters.year.yearMin": "From year",
    "filters.year.yearMax": "To year",
    "filters.year.fromShort": "From",
    "filters.year.toShort": "To",
    "filters.yearsNote": "Titles with no known year are left out.",
    "filters.decade.2020s": "2020s",
    "filters.decade.2010s": "2010s",
    "filters.decade.2000s": "2000s",
    "filters.decade.1990s": "1990s",
    "filters.decade.1980s": "1980s",
    "filters.decade.older": "Older",
    "filters.genres": "Genres",
    "filters.score": "Score",
    "filters.scoreAny": "Any score",
    "filters.scoreAtLeast": "{score} and up",
    "filters.scoreNote": "Titles with no score are left out.",
    "filters.whatsNew": "What's new",
    "filters.stats": "Stats",
    "stats.title": "Stats",
    "stats.tab.growth": "Growth over time",
    "stats.tab.services": "By service",
    "stats.view.table": "Table",
    "stats.view.graph": "Graph",
    "stats.metric.offers": "Added to service",
    "stats.metric.titles": "New to catalog",
    "stats.range.30": "30 days",
    "stats.range.90": "90 days",
    "stats.range.365": "1 year",
    "stats.range.3650": "All",
    "stats.loads": "Include catalog loads",
    "stats.growth.note": "\"Added to service\" counts every title that turned up on a service, even one already on another. \"New to catalog\" counts only titles never seen before, credited to the service that found them first. Syncs that added a quarter or more of what they saw are hidden by default: those loaded a catalog that already existed (a first full sweep, or a widened scrape), rather than new arrivals.",
    "stats.growth.empty": "No syncs in this range",
    "stats.growth.emptyBody": "Try a longer range, or include catalog loads.",
    "stats.growth.byService": "By service",
    "stats.growth.recent": "Recent syncs",
    "stats.growth.recentNote": "The latest {shown} of {total}.",
    "stats.graph.note": "Each service has its own scale, so a small one is not flattened next to a big one. The number in each heading is the total for the range.",
    "stats.graph.peak": "peak {value}",
    "stats.graph.bucket.day": "per day",
    "stats.graph.bucket.week": "per week",
    "stats.graph.bucket.month": "per month",
    "stats.graph.quiet": "{count} services with nothing added in this range are not shown.",
    "stats.col.service": "Service",
    "stats.col.syncs": "Syncs",
    "stats.col.offersAdded": "Added to service",
    "stats.col.titlesCreated": "New to catalog",
    "stats.col.retired": "Removed",
    "stats.col.lastSync": "Last sync",
    "stats.col.when": "When",
    "stats.col.status": "Status",
    "stats.col.titles": "Titles",
    "stats.col.movies": "Movies",
    "stats.col.series": "Series",
    "stats.col.offers": "Offers",
    "stats.col.stream": "Subscription",
    "stats.col.free": "Free",
    "stats.col.rent": "Rent",
    "stats.col.buy": "Buy",
    "stats.col.deal": "Deal",
    "stats.col.priced": "Priced",
    "stats.col.unpriced": "Unpriced",
    "stats.col.coverage": "Coverage",
    "stats.col.titlesPriced": "Titles priced",
    "stats.total": "Total",
    "stats.loadBadge": "catalog load",
    "stats.services.note": "A title to rent and to buy is one title and two offers. A title on several services is counted once in the totals.",
    "stats.prices": "Prices",
    "stats.prices.note": "How many rent and buy offers carry a price. Only services that rent or sell titles appear here.",
    "stats.prices.empty": "No service rents or sells titles right now.",
    "whatsnew.title": "What's new",
    "whatsnew.note":
      "Lately on the services, newest first. A title already on one service counts as new on the service that just added it.",
    "whatsnew.on": "New on {source}",
    "whatsnew.allServices": "All services",
    "whatsnew.empty": "Nothing new here",
    "whatsnew.emptyBody": "No arrivals seen on this service yet. Try another one.",
    "filters.myServices": "My services",
    "filters.myServicesEmpty": "Pick your services",
    "filters.editMine": "Edit my services",
    "filters.editingMine": "Tick the services you have. Each change saves at once.",
    "filters.done": "Done",

    "item.watched": "Watched",
    "item.wantToWatch": "Want to watch",
    "item.rating": "My rating",
    "item.ratingClear": "Clear rating",
    "item.ratingNone": "Not rated yet",
    "item.rate": "Rate {value} out of 10",
    "item.note": "Private note",
    "item.notePlaceholder": "For your eyes only.",
    "item.noteSave": "Save",
    "item.noteSaved": "Saved",
    "item.saveFailed": "That change was not saved.",
    "item.signInToTrack": "Sign in to keep lists and ratings.",
    "item.remove": "Remove from my lists",

    "mylist.title": "My list",
    "mylist.tab.want": "Want to watch",
    "mylist.tab.watched": "Watched",
    "mylist.tab.rated": "Rated",
    "mylist.empty": "Nothing here yet",
    "mylist.emptyBody": "Mark titles as want-to-watch and they show up here.",
    "mylist.browse": "Browse the catalog",
    "mylist.where": "Where you could watch these",
    "mylist.whereOf": "{count} of {total}",

    "settings.title": "Settings",
    "settings.services": "My services",
    "settings.servicesHelp": "Drives the “My services” filter on the catalog.",
    "settings.profile": "Profile",
    "settings.displayName": "Display name",
    "settings.handle": "Handle",
    "settings.handleHelp": "Lowercase letters, digits and underscores.",
    "settings.public": "Public profile",
    "settings.publicHelp":
      "While your profile is public, anyone with the link sees your display name, avatar, " +
      "lists and ratings. Never your email, sign-in identity, notes or chosen services.",
    "settings.save": "Save",
    "settings.saved": "Saved",
    "settings.danger": "Delete account",
    "settings.dangerBody":
      "Immediately deletes your account, lists and ratings. No grace period, no undo.",
    "settings.delete": "Delete my account",
    "settings.deleteConfirm": "Type DELETE to confirm",
    "settings.deleteWord": "DELETE",

    "manage.title": "Manage",
    "manage.tab.overview": "Overview",
    "manage.tab.runs": "Runs",
    "manage.tab.review": "Review queue",
    "manage.tab.members": "Members",
    "members.title": "Who can sign in",
    "members.explain": "Only the addresses on this list can sign in. An address named in the configuration file is always an administrator and cannot be changed from here.",
    "members.none": "Nobody has been invited yet.",
    "members.email": "Email address",
    "members.emailPlaceholder": "name@example.com",
    "members.role": "Role",
    "members.roleMember": "Member",
    "members.roleAdmin": "Admin",
    "members.invite": "Invite",
    "members.inviteFailed": "That address could not be added.",
    "members.promote": "Make admin",
    "members.demote": "Remove admin",
    "members.remove": "Remove",
    "members.fromConfig": "from config",
    "members.you": "you",
    "members.wallTitle": "This catalog is for members",
    "members.refusedTitle": "Your address is not on the list",
    "members.refusedBody": "Signing in worked, but this catalog is invite-only and your address has not been added to it. Ask the owner to add it - signing in again with the same account will not change this.",
    "members.wallBody": "Sign in to see what is streaming. If you do not have access, ask the owner to add your address.",
    "tokens.title": "API tokens",
    "tokens.explain": "A token lets a script use the API as you. It is shown once and never again.",
    "tokens.none": "You have no tokens.",
    "tokens.name": "What it is for",
    "tokens.namePlaceholder": "laptop script",
    "tokens.create": "Create token",
    "tokens.created": "Copy it now - it cannot be shown again.",
    "tokens.revoke": "Revoke",
    "tokens.neverUsed": "never used",
    "tokens.lastUsed": "last used {when}",
    "manage.denied": "This area is closed",
    "manage.deniedBody": "Only administrators of this instance can open it.",

    "manage.stat.titles": "Titles",
    "manage.stat.scored": "With a score",
    "manage.stat.withPoster": "With a poster",
    "manage.stat.queueCleared": "Queue cleared",
    "manage.stat.ofTitles": "{count} of {total}",
    "manage.stat.missing": "{count} missing",
    "manage.stat.waiting": "{count} waiting",
    "manage.col.source": "Source",
    "manage.col.titles": "Titles",
    "manage.col.poster": "Poster",
    "manage.col.score": "Score",
    "manage.col.queue": "Cleared",
    "manage.col.enriched": "Enriched",
    "manage.source.waitingCount": "{count} awaiting review",
    "manage.source.queued": "Sync queued",
    "manage.stat.people": "People",
    "manage.stat.available": "Available now",
    "manage.stat.offers": "Ways to watch",
    "manage.stat.pending": "Awaiting review",
    "manage.stat.stale": "Stale sources",
    "manage.stat.never": "Never",
    "manage.staleOf": "{stale} of {total}",

    "manage.scoring": "What the score is made of",
    "manage.scoring.blurb":
      "How much each rater actually weighed in the catalog's scores: its weight, "
      + "times the number of scored titles it has managed to rate. A thinly voted "
      + "rating counts half, exactly as it does in the sum itself.",
    "manage.scoring.empty": "Nothing has been scored yet, so there is no whole to take a share of.",
    "manage.scoring.israeli": "Israeli score",
    "manage.col.provider": "Rater",
    "manage.col.weight": "Weight",
    "manage.col.share": "Share of the score",
    "manage.col.covered": "Catalog rated",
    "manage.scoring.unused": "Has rated nothing",

    "manage.sources": "Sources",
    "manage.source.coverage": "Coverage",
    "manage.source.lastSync": "Last sync",
    "manage.source.pending": "Queued",
    "manage.source.enabled": "On",
    "manage.source.on": "On",
    "manage.source.off": "Off",
    "manage.source.fromConfig": "Not set by hand",
    "manage.source.override": "Set by hand",
    "manage.source.clearOverride": "Hand back to the file",
    "manage.source.stale": "Stale",
    "manage.source.retired": "No longer tracked",
    "manage.source.nextRun": "Takes effect on the next run",

    "manage.runs.phase": "Phase",
    "manage.runs.status": "Status",
    "manage.runs.started": "Started",
    "manage.runs.duration": "Took",
    "manage.runs.source": "Source",
    "manage.runs.log": "Log",
    "manage.runs.showLog": "Show the log",
    "manage.runs.hideLog": "Close",
    "manage.runs.noLog": "This run left no log",
    "manage.runs.empty": "Nothing has run yet",
    "manage.runs.all": "All",
    "manage.runs.more": "More",

    "review.title": "Review queue",
    "review.empty": "The queue is empty",
    "review.emptyBody": "Everything the sources offered is in the catalog.",
    "review.order.age": "Oldest first",
    "review.order.similarity": "Closest first",
    "review.allSources": "All sources",
    "review.source": "The source is offering",
    "review.candidate": "Might be",
    "review.noCandidate": "No similar title found",
    "review.kind.movie": "Film",
    "review.kind.series": "Series",
    "review.similarity": "{value}% match",
    "review.same": "Same title",
    "review.different": "New title",
    "review.junk": "Not a title",
    "review.open": "Open at the source",
    "review.remaining": "{count} waiting",
    "review.ruled": "Done",
    "review.failed": "That ruling could not be saved",
    "review.selected": "{count} selected",
    "review.bulkDismiss": "Mark all as not titles",
    "review.bulkCreate": "Give each its own title",
    "review.clearSelection": "Clear the selection",
    "review.select": "Select",
    "review.keys": "1 same · 2 new title · 3 not a title · j/k to move",
  },
};

/** The writing direction for a language. */
export function directionOf(language) {
  return language === "he" ? "rtl" : "ltr";
}

/** Whether a language is one we actually ship strings for. */
export function isSupported(language) {
  return LANGUAGES.includes(language);
}

/**
 * Look up a string, substituting {placeholders}.
 *
 * An unknown key returns the key itself rather than throwing or rendering
 * blank: a missing translation should be obvious in the UI, not invisible.
 */
export function translate(language, key, values = {}) {
  const table = STRINGS[language] ?? STRINGS[DEFAULT_LANGUAGE];
  const template = table[key] ?? STRINGS[DEFAULT_LANGUAGE][key] ?? key;

  return template.replace(/\{(\w+)\}/g, (match, name) =>
    Object.hasOwn(values, name) ? String(values[name]) : match,
  );
}

/** Bind a language so views can call `t("key")`. */
export function translator(language) {
  return (key, values) => translate(language, key, values);
}

/**
 * The name to show for a title in this language.
 *
 * Falls back to the other language rather than showing nothing: plenty of
 * Israeli titles have no English name, and a few foreign ones have no Hebrew.
 */
export function displayName(title, language) {
  const preferred = language === "he" ? title.name_he : title.name_en;
  return preferred || title.name_en || title.name_he || "";
}

/** The other name, when it differs from the one already shown. */
export function secondaryName(title, language) {
  const shown = displayName(title, language);
  const other = language === "he" ? title.name_en : title.name_he;
  return other && other !== shown ? other : "";
}
