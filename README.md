...
[![tests](https://img.shields.io/badge/tests-2402%20passing-brightgreen)](https://github.com/barakbl/eifo/actions/workflows/ci.yml)
...
| **Lev VOD** (לב בבית) | ✅ Yes | `lev_vod` | The Lev cinema chain's rental library: ~360 arthouse films at ₪19.90, watchable for 48 hours. The same ticketing platform as the Cinematheque, but this one answers in JSON - one request lists the whole chain, and the VOD rows are the ones at its virtual venue. Two requests per film after that: its own record (release year, countries, director and cast, all in Hebrew) and its price. Posters cost nothing, being addressed by film rather than by filename. Nearly every title names itself in English too, which is the difference between a matcher guessing and being told. |
| **Israel Film Archive** (Jerusalem) | ✅ Yes | `israel_film_archive` | The national archive streaming its own digitised collection: 969 films from 1920 onwards, about half of them free to watch and badged free; the rest rent at ₪15, priced from their own pages. Two shelves, and both are read - the Israeli collection under `/movie/` and the international one it licenses under `/general_film/`, which is where Tarkovsky and Kurosawa live. Nothing on the site lists the collection, so this is the heaviest sync here - the sitemaps, then one page per film, about 16 minutes at one request a second. |
