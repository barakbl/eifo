...
from eifo_api.security import LoginNotConfiguredError
...
class Identity:
    ...
    ``email`` is optional throughout: X frequently omits it, and Eifo neither
...
def _secret(settings: Settings, name: str) -> str:
    if not _is_set(settings, name):
        raise LoginNotConfiguredError(f"EIFO_{name.upper()}")
